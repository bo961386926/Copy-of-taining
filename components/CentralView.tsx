...
const CentralView: React.FC<CentralViewProps> = ({ onEnterSystem, currentProject }) => {
    ...
    <div className="flex-1 h-full relative overflow-hidden bg-[#0f172a] grid-bg perspective-1000">
      ...
      <div className="absolute top-6 left-1/2 -translate-x-1/2 z-20 w-[95%] max-w-3xl text-center pointer-events-none">
        <div className="inline-block relative px-8 py-3 bg-[#0f172a]/80 backdrop-blur-md border-x border-cyan-400/40 rounded-sm">
            ...
            <div className="flex items-center justify-center space-x-2">
               ...
               <p className="text-[10px] md:text-[11px] text-cyan-400 font-bold tracking-[0.2em] uppercase">
                 虹吸式屋面雨水监测运维系统
               </p>
            </div>
        </div>
          ...
          <div key={i} className="absolute z-30 transform -translate-x-1/2 -translate-y-1/2" style={{ top: marker.top, left: marker.left }}>
            ...
            >
              ...
              <div className="relative">
                 <div className={`w-3 h-3 rounded-full border-2 border-white shadow-[0_0_10px_currentColor] transition-all duration-300 ${colorClass} ${activeHoverMarker === i ? 'scale-125' : 'scale-100'}`}>
                 </div>
                 {/* Ripple Effect */}
              ...
              <div className={`absolute bottom-full mb-4 transition-all duration-300 ${activeHoverMarker === i ? 'mb-10 scale-110 z-50' : 'mb-4 scale-100'}`}>
                 <div className={`px-3 py-1.5 bg-[#0f172a]/90 backdrop-blur-md border border-white/30 rounded-sm shadow-xl flex flex-col items-center
                   ${activeHoverMarker === i ? `${borderColor} ${bgColor}` : ''}`}>
                    ...
                    {activeHoverMarker === i && (
                      <div className="mt-1 flex items-center space-x-1 border-t border-white/20 pt-1 w-full justify-center">
                         <span className={`text-[9px] font-mono ${valColor}`}>{marker.val}</span>
                      </div>
                    )}
                 </div>
              </div>
      ...
      <div className="absolute bottom-6 right-6 z-40">
        <button 
          ...
          className="flex items-center space-x-2 bg-gradient-to-r from-cyan-600/90 to-blue-600/90 hover:from-cyan-500 hover:to-blue-500 border border-cyan-400/50 text-white px-6 py-2 rounded-sm transition-all duration-300 shadow-[0_0_20px_rgba(0,229,255,0.2)] hover:shadow-[0_0_30px_rgba(0,229,255,0.4)] group backdrop-blur-md"
        >
          <LogIn size={16} className="text-cyan-200 group-hover:text-white transition-colors" />
          <span className="text-xs font-black tracking-widest">进入监测系统</span>
        </button>
      </div>
